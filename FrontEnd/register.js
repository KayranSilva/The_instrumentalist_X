...
(() => {
    ...
    function error(id, message) { const element = document.getElementById(id); if (element) element.textContent = message; }
    ...
    form === null || form === void 0 ? void 0 : form.addEventListener("submit", async (event) => {
        ...
        let valid = true;
        if (!name) {
            error("nameError", "Informe seu nome.");
            valid = false;
        }
        if (!validEmail(email)) {
            error("emailError", "Digite um e-mail válido.");
            valid = false;
        }
        if (passwordInput.value.length < 6) {
            error("passwordError", "A senha deve ter pelo menos 6 caracteres.");
            valid = false;
        }
        if (passwordInput.value !== confirmInput.value) {
            error("confirmPasswordError", "As senhas não coincidem.");
            valid = false;
        }
        if (!valid)
            return;
        try {
            const response = await fetch(`${API_URL}/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, password: passwordInput.value }) });
            ...
            if (status)
                status.textContent = data.message || "Conta criada com sucesso.";
            if (data.user)
